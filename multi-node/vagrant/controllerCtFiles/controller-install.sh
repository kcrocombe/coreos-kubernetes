...
ENV_FILE=/etc/kubernetes/options.env
...
#Installation should now complete ok, so disable the service so that it doesn't run again
#on next reboot.
systemctl disable kubernetes-install
 
# Would like to mask the service, but we can't if it is still running.
#; systemctl mask kubernetes-install
